...
describe('decrypt', () => {
  ...
  it('{value:string} + {secret}', () => {
    ...
    expect(decrypt('U2FsdGVkX18t06X2N3GF+/l4nQJZ/6jp0BhTvGd6Qxg=', secret2)).toEqual(valueStr);
  });
  ...
  it('{value:object} + {secret}', () => {
    ...
    expect(decrypt('U2FsdGVkX1/CRux2o9ukPAx0zNZw0m/icS3bqacRo/MT3LDLWRSWs0m6ipC476Zk', secret2)).toEqual(valueObj);
  });
  ...
  it('{value:date} + {secret}', () => {
    expect(decrypt('U2FsdGVkX1/BvZgquYu9Epg+j+sRYEXF6pGmLhVv+BA5HTMtiS+YqzOeq/h+I+7f', secret))
      .toEqual('1980-04-09T17:00:00.000Z');
  });

  it('{value:string} + invalid {secret}', () => {
    expect(() => {
      decrypt(block, `${secret}__`);
    }).toThrowError(`Hash ${block} can't be decrypted.`);
  });
});
